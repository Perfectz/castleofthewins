...
- Folder path: `artifacts/menu-redesign-series/step-09-new-game-screen-simplification/`
- Three-sentence summary: This step returns to the highest-friction onboarding surface and simplifies the `Create Adventurer` screen as a phone-first minimalism pass. Live before and after captures showed that the biggest gains came from removing low-priority default content, shortening the preview, and compacting race and class cards without breaking the single-screen flow. The result is a faster-start creation screen that keeps progression context available through an inline `Legacy & Contracts` reveal instead of forcing it into the primary scan path.
- Main changes: removal of the creation-screen audio banner and `Run Notes`, compact creation-specific race and class cards, shortened preview content centered on identity plus five key derived stats, and collapsed-by-default `Legacy & Contracts` disclosure
- Key screenshots: `screenshots/before/before-mobile-02-default.png`, `screenshots/after/after-mobile-02-default.png`, `screenshots/before/before-mobile-05-legacy.png`, `screenshots/after/after-mobile-05-legacy.png`, `screenshots/before/before-desktop-02-default.png`, `screenshots/after/after-desktop-02-default.png`
- What this step proved: Minimalism on the new-game screen is mostly a hierarchy decision. The creation flow became more readable by showing less at once, not by adding guidance or splitting the flow into more surfaces.
- What should happen next: Evaluate whether the attribute allocator itself needs a lighter presentation, and test whether advanced players miss the removed full derived-stat grid enough to justify an optional expanded preview mode.
