...
const STEP_DIR = path.join(ROOT, "artifacts", "menu-performance-series", "step-04-input-latency-and-responsiveness");
...
async function runBaseline(page) {
  ...
  await record("title_to_creation_open", { kind: "click", selector: '[data-action="new-game"]' }, "creation", "01-creation-screen.png");
  await record("creation_to_town_start", { kind: "click", selector: '[data-action="begin-adventure"]' }, "town", "02-town-start.png", { settleFrames: 3 });

  await page.evaluate(() => window.__menuPerf.seedMenuState());

  await record("utility_menu_open_town", { kind: "click", selector: "#utility-menu-button" }, "utility", "03-utility-menu.png");
  await record("utility_to_journal_open", { kind: "click", selector: '[data-action="open-hub"][data-tab="journal"]' }, "journal", "04-journal-current.png");
  await record("journal_section_switch_chronicle", { kind: "click", selector: '[data-action="journal-section"][data-section="chronicle"]' }, "journalChronicle", "05-journal-chronicle.png");
  await record("journal_to_pack_tab_switch", { kind: "click", selector: '[data-action="open-hub"][data-tab="pack"]' }, "pack", "06-pack.png");
  await record("pack_focus_first_item", { kind: "direct", action: "focusBySelector", selector: '[data-action="inspect-pack-item"][data-index="0"]' }, "pack");
  ...
  await record("pack_to_magic_tab_switch", { kind: "click", selector: '[data-action="open-hub"][data-tab="magic"]' }, "magic", "07-magic.png");
  await record("magic_select_identify", { kind: "click", selector: '[data-action="spell-select"][data-spell="identify"]' }, "magicIdentifyActive", "08-magic-identify.png");
  await record("magic_filter_switch_next", { kind: "direct", action: "clickMagicNextFilter" }, "magic", "09-magic-filtered.png");
  await record("magic_escape_close", { kind: "keydown", key: "Escape" }, "modalClosedToGame");

  await record("provisioner_open_direct", { kind: "direct", action: "openTownService", service: "general" }, "shopGeneral", "10-provisioner-buy.png");
  await record("provisioner_panel_switch_sell", { kind: "click", selector: '[data-action="shop-panel"][data-panel="sell"]' }, "shopSellPanel", "11-provisioner-sell.png");
  await record("provisioner_escape_close", { kind: "keydown", key: "Escape" }, "modalClosedToGame");

  await record("sage_open_low_gold", { kind: "direct", action: "openSageLowGold", gold: 0 }, "sage", "12-sage-low-gold.png");
  await record("sage_identify_blocked", { kind: "click", selector: '[data-action="service-use"][data-service="identifyAll"]' }, "warningLog", "13-sage-blocked.png");
