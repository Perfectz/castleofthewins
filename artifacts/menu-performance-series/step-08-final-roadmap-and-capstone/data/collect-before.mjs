...
const STEP_DIR = path.join(ROOT, "artifacts", "menu-performance-series", "step-08-final-roadmap-and-capstone");
  ...
  await page.evaluate(() => {
    ...
    const getUiSnapshot = (label = "") => {
      ...
      return {
        ...
        packGroupCount: modalRoot?.querySelectorAll(".pack-group").length || 0,
        spellButtonCount: modalRoot?.querySelectorAll(".magic-grid button").length || 0,
        magicCardCount: modalRoot?.querySelectorAll(".magic-card").length || 0,
        journalLogCount: modalRoot?.querySelectorAll(".journal-log .log-line").length || 0,
        journalSectionCount: modalRoot?.querySelectorAll(".journal-section-row .hub-filter-chip").length || 0,
        shopRowCount: modalRoot?.querySelectorAll(".shop-row").length || 0,
    ...
    const readyPredicates = {
      ...
      journalMission: () => game().mode === "modal" && game().modalSurfaceKey === "hub:journal" && game().activeJournalSection === "mission",
...
async function runBaseline(page) {
  ...
  await record("title_to_creation_open", { kind: "click", selector: '[data-action="new-game"]' }, "");
  ...
  await record("utility_menu_open_town", { kind: "click", selector: "#utility-menu-button" }, "utility");
  await record("utility_to_journal_open", { kind: "click", selector: '[data-action="open-hub"][data-tab="journal"]' }, "journal", "01-journal-current.png");
  await record("journal_section_switch_chronicle", { kind: "click", selector: '[data-action="journal-section"][data-section="chronicle"]' }, "journalChronicle", "02-journal-chronicle.png");
  await record("journal_section_switch_mission", { kind: "click", selector: '[data-action="journal-section"][data-section="mission"]' }, "journalMission");
  await record("journal_to_pack_tab_switch", { kind: "click", selector: '[data-action="open-hub"][data-tab="pack"]' }, "pack", "03-pack.png");
  await record("pack_filter_switch_use", { kind: "click", selector: '[data-action="pack-filter"][data-filter="use"]' }, "pack", "04-pack-use-filter.png");
  await record("pack_select_second_item", { kind: "direct", action: "clickSecondPackRow" }, "packSelectionChanged", "05-pack-selected.png");
  await record("pack_to_magic_tab_switch", { kind: "click", selector: '[data-action="open-hub"][data-tab="magic"]' }, "magic", "06-magic.png");
  await record("magic_filter_switch_next", { kind: "direct", action: "clickMagicNextFilter" }, "magic", "07-magic-filtered.png");
  await record("magic_escape_close", { kind: "keydown", key: "Escape" }, "modalClosedToGame");

  await record("provisioner_open_direct", { kind: "direct", action: "openTownService", service: "general" }, "shopGeneral", "08-provisioner-buy.png");
  await record("provisioner_panel_switch_sell", { kind: "click", selector: '[data-action="shop-panel"][data-panel="sell"]' }, "shopSellPanel", "09-provisioner-sell.png");
  ...
  await record("sage_identify_blocked", { kind: "click", selector: '[data-action="service-use"][data-service="identifyAll"]' }, "", "11-sage-blocked.png");
...
async function main() {
  ...
  try {
    ...
    await writeFile(path.join(DATA_DIR, "before-raw-metrics.json"), `${JSON.stringify(payload, null, 2)}\n`, "utf8");
    console.log(path.join(DATA_DIR, "before-raw-metrics.json"));
  } finally {
    if (browser) {
      await browser.close();
    }
