...
const STEP_DIR = path.join(ROOT, "artifacts", "menu-performance-series", "step-03-render-cost-and-dom-churn");
...
async function installProfiler(page) {
  await page.evaluate(() => {
    ...
    const methodNames = [
      ...
      "buildMagicHubViewModel",
      "buildJournalHubViewModel",
      "showShopModal",
...
async function runBaseline(page) {
  ...
  await record("utility_to_journal_open", { kind: "click", selector: '[data-action="open-hub"][data-tab="journal"]' }, "journal", "06-journal-current.png");
  await record("journal_section_switch_chronicle", { kind: "click", selector: '[data-action="journal-section"][data-section="chronicle"]' }, "journal", "07-journal-chronicle.png");
  ...
  await record("pack_select_second_item", { kind: "direct", action: "clickSecondPackRow" }, "pack");
  await record("pack_to_magic_tab_switch", { kind: "click", selector: '[data-action="open-hub"][data-tab="magic"]' }, "magic", "10-magic.png");
  await record("magic_select_identify", { kind: "click", selector: '[data-action="spell-select"][data-spell="identify"]' }, "magic");
  await record("magic_filter_switch_next", { kind: "direct", action: "clickMagicNextFilter" }, "magic", "11-magic-filtered.png");
  ...
  await record("provisioner_open_direct", { kind: "direct", action: "openTownService", service: "general" }, "shopGeneral", "12-provisioner-buy.png");
  await record("provisioner_panel_switch_sell", { kind: "click", selector: '[data-action="shop-panel"][data-panel="sell"]' }, "shopSellPanel", "13-provisioner-sell.png");
  await record("provisioner_close_to_game", { kind: "click", selector: '[data-action="close-modal"]' }, "modalClosedToGame");

  await record("town_bank_open_direct", { kind: "direct", action: "showBankModal" }, "bank", "14-town-bank.png");
